...
module relin_merged_final_op_hadamard
   #(   
        parameter LOGK       = 10,
        parameter LOGQ       = 64,
        parameter LOGQH      = 48,
...
localparam OPCODE_RELIN = 1'b0;
localparam OPCODE_HADAMARD = 1'b1;
...
localparam LAT_HADAMARD = LAT_MUL + 1;
...
shift_reg #(
    ...
    .WIDTH (1      )
) shift_reg_o_valid_hadamard (
    .clk    (clk                     ),
    .rst    (rst                     ),
    .i_data (o_valid_hadamard        ),
    .o_data (o_valid_shifted_hadamard)
);
...
for (genvar i = 0; i < TP; i = i + 1) begin
    assign modadd_in_A[i] = (opcode == OPCODE_HADAMARD) ? '0 : ((last_q) ? A_d[i] : modmul_out[i]);
    assign modadd_in_B[i] = (opcode == OPCODE_HADAMARD) ? '0 : ((last_q) ? halfmod_q : B_d[i]);
end
...
for (genvar i = 0; i < TP; i = i + 1) begin
    assign modsub0_in_A[i] = (opcode == OPCODE_HADAMARD) ? '0 : bram_out[i];
    assign modsub0_in_B[i] = (opcode == OPCODE_HADAMARD) ? '0 : halfmod_q;
end

for (genvar i = 0; i < TP; i = i + 1) begin
    assign modsub1_in_A[i] = (opcode == OPCODE_HADAMARD) ? '0 : A_d[i];
    assign modsub1_in_B[i] = (opcode == OPCODE_HADAMARD) ? '0 : modsub0_out[i];
end

for (genvar i = 0; i < TP; i = i + 1) begin
    assign modmul_in_A[i] = (opcode == OPCODE_HADAMARD) ? A[i] : modsub1_out[i];
    assign modmul_in_B[i] = (opcode == OPCODE_HADAMARD) ? B[i] : q_inv_q;
end
