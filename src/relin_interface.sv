`ifndef RELIN_INTERFACE
`define RELIN_INTERFACE
...
interface relin_t #(
  ...
  logic [$clog(L)  -1:0]    rw_id            ;
  logic                     r_psi_inv        ;
  logic                     r_psi_start      ;
  logic                     r_psi_valid      ;
  logic [W         -1:0]    r_psi    [TP-1:0]; 
  logic                     r_psi_done       ;
  logic [$clog(L)  -1:0]    r_poly_id        ;
  logic                     r_poly_start     ;
  logic                     r_poly_valid     ;
  logic [W         -1:0]    r_poly   [TP-1:0]; 
  logic                     r_poly_done      ;
  logic [$clog(L)  -1:0]    r_rlk_id         ;
  logic                     r_rlk_start      ;
  logic                     r_rlk_valid      ;
  logic [W         -1:0]    r_rlk_0  [TP-1:0]; 
  logic [W         -1:0]    r_rlk_1  [TP-1:0];
  logic                     r_rlk_done       ;
  logic                     w_ready          ;
  logic [W         -1:0]    w_poly   [TP-1:0];
  logic                     w_valid          ;
